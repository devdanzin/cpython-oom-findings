...
print("Importing target module: _strptime", file=stderr)
import _strptime
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"_strptime.{method_name}()" if obj_to_call is _strptime else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, _strptime, func_name_str, *arguments, verbose=verbose)
...
print("--- Fuzzing 14 functions in _strptime ---", file=stderr)
# OOM sweep: _strptime_time
oom_call("f1:_strptime._strptime_time", getattr(fuzz_target_module, "_strptime_time"),
    "\x9C/\xDA\x05\xBE\x1C\x81",
    "\u8F51\u928D\u954F\u2826\uC570\u1476\u5EF4\uCCB4\uF061\u9694\uF76D\uDB20\u32AE\uCCF0\u03B6",
)

# OOM sweep: _strptime_time
oom_call("f2:_strptime._strptime_time", getattr(fuzz_target_module, "_strptime_time"),
    "4_ZDGCQqrAylRC2tR2MHHZVO7b19VXU3jZf-c1E5pZJRmwh8Ztpjw75nEW.KEG/Z3/3/",
)

# OOM sweep: _strptime_datetime_time
oom_call("f3:_strptime._strptime_datetime_time", getattr(fuzz_target_module, "_strptime_datetime_time"),
    -514439366278348,
    tuple[weird_classes['weird_dict']] | weird_classes['weird_dict'] | big_union,
)

# OOM sweep: re_compile
oom_call("f4:_strptime.re_compile", getattr(fuzz_target_module, "re_compile"),
    tuple[weird_classes['weird_int']],
    errback,
)

# OOM sweep: _strptime_time
oom_call("f5:_strptime._strptime_time", getattr(fuzz_target_module, "_strptime_time"),
    "\x00",
    16,
)

# OOM sweep: re_sub
oom_call("f6:_strptime.re_sub", getattr(fuzz_target_module, "re_sub"),
    "\x96\xCB\xBD=u\x1B\xCC\x0Aj\x8C\x9E",
    -28.1,
    False,
    -6906174534698,
)
...
# OOM sweep: _parse_tz
oom_call("f7:_strptime._parse_tz", getattr(fuzz_target_module, "_parse_tz"),
    None,
    b"\x99\x24\xCB\xAB\xB6\x0C\xE0\x05\x7A\xF9\xFD\x47\xAD\xC6\xFC\x0C\x91\x51\xD1\xA1",
    tuple[weird_classes['weird_frozenset']],
)

# OOM sweep: _strptime_time
oom_call("f8:_strptime._strptime_time", getattr(fuzz_target_module, "_strptime_time"),
    weird_instances['weird_dict_empty'],
    "\udbff\udfff",
)

# OOM sweep: _fixmonths
oom_call("f9:_strptime._fixmonths", getattr(fuzz_target_module, "_fixmonths"),
    errback,
)

# OOM sweep: _getlang
oom_call("f10:_strptime._getlang", getattr(fuzz_target_module, "_getlang"),
)
