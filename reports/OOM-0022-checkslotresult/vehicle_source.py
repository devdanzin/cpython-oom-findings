...
# OOM sweep: find_function
oom_call("f1:pdb.find_function", getattr(fuzz_target_module, "find_function"),
    "\U0010FFFF",
    -844.308,
)

# OOM sweep: runcall
oom_call("f2:pdb.runcall", getattr(fuzz_target_module, "runcall"),
)

# OOM sweep: find_first_executable_line
oom_call("f3:pdb.find_first_executable_line", getattr(fuzz_target_module, "find_first_executable_line"),
    -18,
)

# OOM sweep: _connect
oom_call("f4:pdb._connect", getattr(fuzz_target_module, "_connect"),
)

# OOM sweep: post_mortem
oom_call("f5:pdb.post_mortem", getattr(fuzz_target_module, "post_mortem"),
    int,
)

# OOM sweep: set_trace
oom_call("f6:pdb.set_trace", getattr(fuzz_target_module, "set_trace"),
)

# OOM sweep: post_mortem
oom_call("f7:pdb.post_mortem", getattr(fuzz_target_module, "post_mortem"),
)

# OOM sweep: set_trace
oom_call("f8:pdb.set_trace", getattr(fuzz_target_module, "set_trace"),
)

# OOM sweep: _post_mortem
oom_call("f9:pdb._post_mortem", getattr(fuzz_target_module, "_post_mortem"),
    list[weird_classes['weird_Decimal']] | weird_classes['weird_deque'] | big_union,
    errback,
)

# OOM sweep: _post_mortem
oom_call("f10:pdb._post_mortem", getattr(fuzz_target_module, "_post_mortem"),
    "Dc\xEC",
    b"",
)
