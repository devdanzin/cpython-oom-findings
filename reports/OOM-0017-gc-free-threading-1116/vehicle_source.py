...
# OOM sweep: getprotobyname
oom_call("f1:socket.getprotobyname", getattr(fuzz_target_module, "getprotobyname"),
    None,
)

# OOM sweep: recv_fds
oom_call("f2:socket.recv_fds", getattr(fuzz_target_module, "recv_fds"),
    b"\xBE\xC4\x4B\xA0\xBE\x8B\x8F\x6A\xE3\x7F\x3D\x89\xBF\xCC\x1F\xEC\xF7\xFB\x06",
    74345403108817,
    "gVSQeAZ/../S3eBNS8hc1V_ict6y3J9JOyjWd/R/G5/K",
)

# OOM sweep: dup
oom_call("f3:socket.dup", getattr(fuzz_target_module, "dup"),
    "\x00",
)

# OOM sweep: send_fds
oom_call("f4:socket.send_fds", getattr(fuzz_target_module, "send_fds"),
    Exception('fuzzer_generated_exception'),
    list[weird_classes['weird_Counter']],
    '/etc/machine-id',
)

# OOM sweep: recv_fds
oom_call("f5:socket.recv_fds", getattr(fuzz_target_module, "recv_fds"),
    "\x00",
    None,
    weird_instances['weird_int_2**31'],
)

# OOM sweep: inet_aton
oom_call("f6:socket.inet_aton", getattr(fuzz_target_module, "inet_aton"),
    -2,
)

# OOM sweep: getservbyport
oom_call("f7:socket.getservbyport", getattr(fuzz_target_module, "getservbyport"),
    bytes(10 ** 5),
    -3550112432,
)

# OOM sweep: dup
oom_call("f8:socket.dup", getattr(fuzz_target_module, "dup"),
    weird_instances['weird_int_-2**63'],
)

# OOM sweep: if_nametoindex
oom_call("f9:socket.if_nametoindex", getattr(fuzz_target_module, "if_nametoindex"),
    MagicMock(),
)

# OOM sweep: inet_ntoa
oom_call("f10:socket.inet_ntoa", getattr(fuzz_target_module, "inet_ntoa"),
    list[weird_classes['weird_bytes']] | weird_classes['weird_OrderedDict'] | big_union,
)
