...
print("Importing target module: concurrent.interpreters", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"concurrent.interpreters.{method_name}()" if obj_to_call is concurrent.interpreters else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, concurrent.interpreters, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = concurrent.interpreters
...
print("--- Fuzzing 6 functions in concurrent.interpreters ---", file=stderr)
# OOM sequence: get_current > get_current > create_queue
def _oom_seq_f1():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:get_current", file=stderr)
        getattr(fuzz_target_module, "get_current", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:get_current", file=stderr)
        getattr(fuzz_target_module, "get_current", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:create_queue", file=stderr)
        getattr(fuzz_target_module, "create_queue", None)(
            "\U0010FFFF",
        )
    except BaseException:
        pass
oom_run("f1:concurrent.interpreters[get_current>get_current>create_queue]", _oom_seq_f1)

# OOM sequence: get_main > get_current > is_shareable
def _oom_seq_f2():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:get_main", file=stderr)
        getattr(fuzz_target_module, "get_main", None)(
        )
    except BaseException:
        pass
    try:
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:is_shareable", file=stderr)
        getattr(fuzz_target_module, "is_shareable", None)(
            "\xA5@\xFA-",
        )
    except BaseException:
        pass
oom_run("f2:concurrent.interpreters[get_main>get_current>is_shareable]", _oom_seq_f2)

# OOM sequence: get_main > create > is_shareable
def _oom_seq_f3():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:get_main", file=stderr)
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:create", file=stderr)
        getattr(fuzz_target_module, "create", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:is_shareable", file=stderr)
        getattr(fuzz_target_module, "is_shareable", None)(
            Template("\x00", Interpolation(weird_instances['weird_Queue_special'], "name")),
        )
    except BaseException:
        pass
oom_run("f3:concurrent.interpreters[get_main>create>is_shareable]", _oom_seq_f3)

# OOM sequence: create > list_all > create
def _oom_seq_f4():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:create", file=stderr)
        getattr(fuzz_target_module, "create", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:list_all", file=stderr)
        getattr(fuzz_target_module, "list_all", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:create", file=stderr)
        getattr(fuzz_target_module, "create", None)(
        )
    except BaseException:
        pass
oom_run("f4:concurrent.interpreters[create>list_all>create]", _oom_seq_f4)

# OOM sequence: list_all > get_main > create_queue
def _oom_seq_f5():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:list_all", file=stderr)
        getattr(fuzz_target_module, "list_all", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:get_main", file=stderr)
        getattr(fuzz_target_module, "get_main", None)(
        )
    except BaseException:
        pass
    try:
        ...
        pass
oom_run("f5:concurrent.interpreters[list_all>get_main>create_queue]", _oom_seq_f5)

# OOM sequence: create_queue > create_queue > get_main
def _oom_seq_f6():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:create_queue", file=stderr)
        getattr(fuzz_target_module, "create_queue", None)(
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:create_queue", file=stderr)
        getattr(fuzz_target_module, "create_queue", None)(
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:get_main", file=stderr)
        getattr(fuzz_target_module, "get_main", None)(
        )
    except BaseException:
        pass
oom_run("f6:concurrent.interpreters[create_queue>create_queue>get_main]", _oom_seq_f6)

# OOM sequence: list_all > is_shareable > list_all
def _oom_seq_f7():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:list_all", file=stderr)
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:is_shareable", file=stderr)
        getattr(fuzz_target_module, "is_shareable", None)(
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:list_all", file=stderr)
        getattr(fuzz_target_module, "list_all", None)(
        )
    except BaseException:
        pass
oom_run("f7:concurrent.interpreters[list_all>is_shareable>list_all]", _oom_seq_f7)

# OOM sequence: list_all > is_shareable > get_current
def _oom_seq_f8():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:list_all", file=stderr)
    ...
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s2:is_shareable", file=stderr)
        getattr(fuzz_target_module, "is_shareable", None)(
            dict[weird_classes['weird_dict']],
        )
    except BaseException:
        pass
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s3:get_current", file=stderr)
        getattr(fuzz_target_module, "get_current", None)(
        )
    except BaseException:
        pass
oom_run("f8:concurrent.interpreters[list_all>is_shareable>get_current]", _oom_seq_f8)

# OOM sequence: create > list_all > create_queue
def _oom_seq_f9():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:create", file=stderr)
    ...
    try:
        ...
        pass
oom_run("f9:concurrent.interpreters[create>list_all>create_queue]", _oom_seq_f9)

# OOM sequence: list_all > is_shareable > get_main
def _oom_seq_f10():
    try:
        if _OOM_VERBOSE:
            print("[OOM-SEQ]     step s1:list_all", file=stderr)
    ...
    try:
        ...
        pass
oom_run("f10:concurrent.interpreters[list_all>is_shareable>get_main]", _oom_seq_f10)


print("--- OOM-fuzzing 2 classes in concurrent.interpreters ---", file=stderr)
# OOM sweep: Interpreter() constructor
oom_call("oc1:concurrent.interpreters.Interpreter", getattr(fuzz_target_module, "Interpreter", None),
)

oom_inst_oc1_interpreter = None
try:
    oom_inst_oc1_interpreter = callFunc("oc1_init", "Interpreter",
    )
except Exception:
    oom_inst_oc1_interpreter = None

if oom_inst_oc1_interpreter is not None and oom_inst_oc1_interpreter is not SENTINEL_VALUE:
    # OOM sequence on Interpreter: __sizeof__ > prepare_main > __subclasshook__
    def _oom_seq_oc1():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__sizeof__", file=stderr)
            getattr(oom_inst_oc1_interpreter, "__sizeof__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:prepare_main", file=stderr)
            getattr(oom_inst_oc1_interpreter, "prepare_main", None)(
                b"\x9F\x78\xE2\x4F\xB0\xFD\xF1\xC8",
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m3:__subclasshook__", file=stderr)
            getattr(oom_inst_oc1_interpreter, "__subclasshook__", None)(
                '/etc/machine-id',
            )
        except BaseException:
            pass
    oom_run("oc1:concurrent.interpreters.Interpreter[__sizeof__>prepare_main>__subclasshook__]", _oom_seq_oc1)

    del oom_inst_oc1_interpreter
    collect()

# OOM sweep: Queue() constructor
oom_call("oc2:concurrent.interpreters.Queue", getattr(fuzz_target_module, "Queue", None),
)

oom_inst_oc2_queue = None
try:
    oom_inst_oc2_queue = callFunc("oc2_init", "Queue",
    )
except Exception:
    oom_inst_oc2_queue = None

if oom_inst_oc2_queue is not None and oom_inst_oc2_queue is not SENTINEL_VALUE:
    # OOM sequence on Queue: __le__ > full > __reduce_ex__
    def _oom_seq_oc2():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__le__", file=stderr)
            getattr(oom_inst_oc2_queue, "__le__", None)(
        ...
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:full", file=stderr)
            getattr(oom_inst_oc2_queue, "full", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m3:__reduce_ex__", file=stderr)
            getattr(oom_inst_oc2_queue, "__reduce_ex__", None)(
            )
        except BaseException:
            pass
    oom_run("oc2:concurrent.interpreters.Queue[__le__>full>__reduce_ex__]", _oom_seq_oc2)

    del oom_inst_oc2_queue
    collect()

# OOM sweep: Queue() constructor
oom_call("oc3:concurrent.interpreters.Queue", getattr(fuzz_target_module, "Queue", None),
)

oom_inst_oc3_queue = None
try:
    oom_inst_oc3_queue = callFunc("oc3_init", "Queue",
    )
except Exception:
    oom_inst_oc3_queue = None

if oom_inst_oc3_queue is not None and oom_inst_oc3_queue is not SENTINEL_VALUE:
    # OOM sequence on Queue: __setattr__ > __setattr__ > __ne__
    def _oom_seq_oc3():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__setattr__", file=stderr)
            getattr(oom_inst_oc3_queue, "__setattr__", None)(
                "\x1B\x8B\x86\xDC\x15p\x09L\xAD\xCB\xBB\xE1\xB3\x0D\x81t",
                16,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:__setattr__", file=stderr)
            getattr(oom_inst_oc3_queue, "__setattr__", None)(
                bytearray(b"test"),
        ...
        try:
            ...
            getattr(oom_inst_oc3_queue, "__ne__", None)(
                -39.3203,
            )
        except BaseException:
            pass
    oom_run("oc3:concurrent.interpreters.Queue[__setattr__>__setattr__>__ne__]", _oom_seq_oc3)

    del oom_inst_oc3_queue
    collect()

# OOM sweep: Queue() constructor
oom_call("oc4:concurrent.interpreters.Queue", getattr(fuzz_target_module, "Queue", None),
)

oom_inst_oc4_queue = None
try:
    oom_inst_oc4_queue = callFunc("oc4_init", "Queue",
    )
except Exception:
    oom_inst_oc4_queue = None

if oom_inst_oc4_queue is not None and oom_inst_oc4_queue is not SENTINEL_VALUE:
    # OOM sequence on Queue: __init_subclass__ > __del__ > __reduce__
    def _oom_seq_oc4():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__init_subclass__", file=stderr)
            getattr(oom_inst_oc4_queue, "__init_subclass__", None)(
                weird_classes['weird_str'],
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:__del__", file=stderr)
            getattr(oom_inst_oc4_queue, "__del__", None)(
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc4_queue, "__reduce__", None)(
            )
        except BaseException:
            pass
    oom_run("oc4:concurrent.interpreters.Queue[__init_subclass__>__del__>__reduce__]", _oom_seq_oc4)

    del oom_inst_oc4_queue
    collect()

# OOM sweep: Interpreter() constructor
oom_call("oc5:concurrent.interpreters.Interpreter", getattr(fuzz_target_module, "Interpreter", None),
)

oom_inst_oc5_interpreter = None
try:
    oom_inst_oc5_interpreter = callFunc("oc5_init", "Interpreter",
    )
except Exception:
    oom_inst_oc5_interpreter = None

if oom_inst_oc5_interpreter is not None and oom_inst_oc5_interpreter is not SENTINEL_VALUE:
    # OOM sequence on Interpreter: call > __repr__ > is_running
    def _oom_seq_oc5():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:call", file=stderr)
            getattr(oom_inst_oc5_interpreter, "call", None)(
                0.9,
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc5_interpreter, "__repr__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m3:is_running", file=stderr)
            getattr(oom_inst_oc5_interpreter, "is_running", None)(
            )
        except BaseException:
            pass
    oom_run("oc5:concurrent.interpreters.Interpreter[call>__repr__>is_running]", _oom_seq_oc5)

    del oom_inst_oc5_interpreter
    collect()
