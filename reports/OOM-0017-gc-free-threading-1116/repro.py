...
Minimal-ish reproducer: abort on
    Python/gc_free_threading.c:1116
    validate_gc_objects: Assertion "gc_get_refs(op) >= 0" failed: refcount is too small

Affected:   CPython 3.16.0a0 (main, commit 15d7406).
Crash:      SIGABRT (rc 134) during interpreter finalization GC.
            The corrupted object is the `array.array` *type* object
            (object type name: type, object repr: <class 'array.array'>).
Builds:     - free-threaded debug+ASan: abort at gc_free_threading.c:1116 (this file).
            - jit (debug, GIL): abort at Python/gc.c:96 (gc_decref, same root bug).
            - ft_release / upstream: assertions compiled out (NDEBUG) -> clean exit.
Requires:   a debug build exposing _testcapi.set_nomemory.

Run:
    python repro.py        # aborts (rc 134) on the FT debug+ASan build.

What it does
------------
The fuzzer boilerplate establishes a fixed allocation-count baseline, then a
single dense OOM sweep drives socket.recv_fds() with set_nomemory(start, 0) for
start = 0..999. socket.recv_fds() does `import array; array.array("i")` and then
recvmsg(); under OOM the array construction repeatedly fails part-way
(newarrayobject -> tp_alloc takes a deferred reference to the array.array type,
then PyMem_NEW for ob_item fails -> Py_DECREF(op) -> array_dealloc ->
Py_DECREF(tp), arraymodule.c:848). Across the dense sweep the deferred-refcount
bookkeeping for the `array.array` TYPE ends up smaller than the number of live
references the GC later tallies.

At interpreter shutdown, _Py_Finalize -> PyGC_Collect runs the free-threaded
collector. update_refs() seeds gc_refs with the object's refcount and
visit_decref() subtracts one per internal reference; because the type's refcount
is too small, gc_refs underflows below zero and the debug-only consistency
assertion gc_get_refs(op) >= 0 (validate_gc_objects, gc_free_threading.c:1116)
fires and aborts.

Backtrace (gdb, FT debug+ASan):
    #9  validate_gc_objects   Python/gc_free_threading.c:1115  (assert L1116)
...
Minimization note: PARTIAL. The crash is sensitive to allocation alignment
(adding even a single extra allocation before the sweep moves the failing
allocation number and the bug no longer trips), so the reproducer keeps the
fuzzer boilerplate to fix the baseline and reduces the fuzz body to one
recv_fds() OOM sweep. A pure stdlib `import array` / `array.array("i")` sweep in
isolation does not reproduce. Deterministic at this form on the FT debug build.

The reduced source below is from the representative crash directory
(python-5/socket-sigabrt-assertion) with the fuzz body cut to the first sweep.
"""
...
print("Importing target module: socket", file=stderr)
import socket
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"socket.{method_name}()" if obj_to_call is socket else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, socket, func_name_str, *arguments, verbose=verbose)
...
print("--- Fuzzing 32 functions in socket ---", file=stderr)
# OOM sweep: recv_fds
oom_call(getattr(fuzz_target_module, "recv_fds"),
    b"\x3A\x9A\xC7\x40\x80\xA6\xFB",
    -42.65,
    tuple[weird_classes['weird_object']],
    '/etc/machine-id',
)

# OOM sweep: _fallback_socketpair
