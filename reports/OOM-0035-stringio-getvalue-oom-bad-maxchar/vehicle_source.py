...
print("Importing target module: email.iterators", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"email.iterators.{method_name}()" if obj_to_call is email.iterators else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, email.iterators, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = email.iterators
...
print("--- Fuzzing 4 functions in email.iterators ---", file=stderr)
# OOM sweep: typed_subpart_iterator
oom_call("f1:email.iterators.typed_subpart_iterator", getattr(fuzz_target_module, "typed_subpart_iterator"),
    r"w\sq+Jduhg.er...?.\D...snz",
    Exception('fuzzer_generated_exception'),
)

# OOM sweep: typed_subpart_iterator
oom_call("f2:email.iterators.typed_subpart_iterator", getattr(fuzz_target_module, "typed_subpart_iterator"),
    False,
)

# OOM sweep: walk
oom_call("f3:email.iterators.walk", getattr(fuzz_target_module, "walk"),
)

# OOM sweep: body_line_iterator
oom_call("f4:email.iterators.body_line_iterator", getattr(fuzz_target_module, "body_line_iterator"),
    19,
)

# OOM sweep: _structure
oom_call("f5:email.iterators._structure", getattr(fuzz_target_module, "_structure"),
    weird_instances['weird_float_empty'],
    (tricky_property,
     tuple[weird_classes['weird_int']]),
)

# OOM sweep: _structure
oom_call("f6:email.iterators._structure", getattr(fuzz_target_module, "_structure"),
    "\x00" * 10,
)

# OOM sweep: body_line_iterator
oom_call("f7:email.iterators.body_line_iterator", getattr(fuzz_target_module, "body_line_iterator"),
    "\x00",
    [[[[[[[[[[[[[[]]]]]]]]]]]]]],
)

# OOM sweep: typed_subpart_iterator
oom_call("f8:email.iterators.typed_subpart_iterator", getattr(fuzz_target_module, "typed_subpart_iterator"),
    '/bin/sh',
)

# OOM sweep: typed_subpart_iterator
oom_call("f9:email.iterators.typed_subpart_iterator", getattr(fuzz_target_module, "typed_subpart_iterator"),
    None,
)

# OOM sweep: _structure
oom_call("f10:email.iterators._structure", getattr(fuzz_target_module, "_structure"),
    TrickyDescriptor,
    errback,
...
print("--- OOM-fuzzing 1 classes in email.iterators ---", file=stderr)
# OOM sweep: StringIO() constructor
oom_call("oc1:email.iterators.StringIO", getattr(fuzz_target_module, "StringIO", None),
)

oom_inst_oc1_stringio = None
try:
    oom_inst_oc1_stringio = callFunc("oc1_init", "StringIO",
    )
except Exception:
    oom_inst_oc1_stringio = None

if oom_inst_oc1_stringio is not None and oom_inst_oc1_stringio is not SENTINEL_VALUE:
    # OOM sweep: StringIO.__format__()
    oom_call("oc1m1:email.iterators.StringIO.__format__", getattr(oom_inst_oc1_stringio, "__format__", None),
        r"VVRD*Q.zr\W..x..\BX\sv\s?V?\Z\S",
    )
    # OOM sweep: StringIO.__reduce__()
    oom_call("oc1m2:email.iterators.StringIO.__reduce__", getattr(oom_inst_oc1_stringio, "__reduce__", None),
    )
    # OOM sweep: StringIO.writelines()
    oom_call("oc1m3:email.iterators.StringIO.writelines", getattr(oom_inst_oc1_stringio, "writelines", None),
        r"KH.x?t.vNeM\SII*HAKnQu.B.\Aum.*uu",
    )
    # OOM sweep: StringIO.__getstate__()
    oom_call("oc1m4:email.iterators.StringIO.__getstate__", getattr(oom_inst_oc1_stringio, "__getstate__", None),
    )
    # OOM sweep: StringIO.__reduce_ex__()
    oom_call("oc1m5:email.iterators.StringIO.__reduce_ex__", getattr(oom_inst_oc1_stringio, "__reduce_ex__", None),
        b"\xE3\xCC\x78\xFA\x72\xFE\x10\x46\xE1\x64\xB9\x3C\xC7",
    )
    del oom_inst_oc1_stringio
    collect()

# OOM sweep: StringIO() constructor
oom_call("oc2:email.iterators.StringIO", getattr(fuzz_target_module, "StringIO", None),
)

oom_inst_oc2_stringio = None
try:
    oom_inst_oc2_stringio = callFunc("oc2_init", "StringIO",
    )
except Exception:
    oom_inst_oc2_stringio = None

if oom_inst_oc2_stringio is not None and oom_inst_oc2_stringio is not SENTINEL_VALUE:
    # OOM sweep: StringIO.__setstate__()
    oom_call("oc2m1:email.iterators.StringIO.__setstate__", getattr(oom_inst_oc2_stringio, "__setstate__", None),
        Exception('fuzzer_generated_exception'),
    )
    # OOM sweep: StringIO.writable()
    oom_call("oc2m2:email.iterators.StringIO.writable", getattr(oom_inst_oc2_stringio, "writable", None),
    )
    # OOM sweep: StringIO.read()
    oom_call("oc2m3:email.iterators.StringIO.read", getattr(oom_inst_oc2_stringio, "read", None),
    )
    # OOM sweep: StringIO.__new__()
    oom_call("oc2m4:email.iterators.StringIO.__new__", getattr(oom_inst_oc2_stringio, "__new__", None),
        "\x06\xB5H\x0C\x85@\x8F\xE6",
        "\x9EI\xC6\xE8\xC6\xA0K\x9F\xF2!u\x84\x9BGk",
        tricky_module2,
    )
    # OOM sweep: StringIO.isatty()
    oom_call("oc2m5:email.iterators.StringIO.isatty", getattr(oom_inst_oc2_stringio, "isatty", None),
    )
    del oom_inst_oc2_stringio
    collect()

# OOM sweep: StringIO() constructor
oom_call("oc3:email.iterators.StringIO", getattr(fuzz_target_module, "StringIO", None),
)

oom_inst_oc3_stringio = None
try:
    oom_inst_oc3_stringio = callFunc("oc3_init", "StringIO",
    )
except Exception:
    oom_inst_oc3_stringio = None

if oom_inst_oc3_stringio is not None and oom_inst_oc3_stringio is not SENTINEL_VALUE:
    # OOM sweep: StringIO.__reduce_ex__()
    oom_call("oc3m1:email.iterators.StringIO.__reduce_ex__", getattr(oom_inst_oc3_stringio, "__reduce_ex__", None),
    )
    # OOM sweep: StringIO.close()
    oom_call("oc3m2:email.iterators.StringIO.close", getattr(oom_inst_oc3_stringio, "close", None),
    )
    # OOM sweep: StringIO.__setattr__()
    oom_call("oc3m3:email.iterators.StringIO.__setattr__", getattr(oom_inst_oc3_stringio, "__setattr__", None),
        dict[weird_classes['weird_deque']] | weird_classes['weird_float'] | big_union,
        b"\xC7\x2D\xA1\xA3\x26\x09\x4C\xAF\x03\xA6\x04",
    )
    # OOM sweep: StringIO.__delattr__()
    oom_call("oc3m4:email.iterators.StringIO.__delattr__", getattr(oom_inst_oc3_stringio, "__delattr__", None),
        [[[[[[[[[[[[[[]]]]]]]]]]]]]],
    )
    # OOM sweep: StringIO.__init__()
    oom_call("oc3m5:email.iterators.StringIO.__init__", getattr(oom_inst_oc3_stringio, "__init__", None),
    )
    del oom_inst_oc3_stringio
    collect()

# OOM sweep: StringIO() constructor
oom_call("oc4:email.iterators.StringIO", getattr(fuzz_target_module, "StringIO", None),
)

oom_inst_oc4_stringio = None
try:
    oom_inst_oc4_stringio = callFunc("oc4_init", "StringIO",
    )
except Exception:
    oom_inst_oc4_stringio = None

if oom_inst_oc4_stringio is not None and oom_inst_oc4_stringio is not SENTINEL_VALUE:
    # OOM sweep: StringIO._checkClosed()
    oom_call("oc4m1:email.iterators.StringIO._checkClosed", getattr(oom_inst_oc4_stringio, "_checkClosed", None),
    )
    # OOM sweep: StringIO.__getattribute__()
    oom_call("oc4m2:email.iterators.StringIO.__getattribute__", getattr(oom_inst_oc4_stringio, "__getattribute__", None),
        dict[weird_classes['weird_dict']] | weird_classes['weird_bytes'] | big_union,
    )
    # OOM sweep: StringIO.__exit__()
    oom_call("oc4m3:email.iterators.StringIO.__exit__", getattr(oom_inst_oc4_stringio, "__exit__", None),
        dict[weird_classes['weird_float']],
        "/",
        '/etc/machine-id',
    )
    # OOM sweep: StringIO.writable()
    oom_call("oc4m4:email.iterators.StringIO.writable", getattr(oom_inst_oc4_stringio, "writable", None),
    )
    # OOM sweep: StringIO.isatty()
    oom_call("oc4m5:email.iterators.StringIO.isatty", getattr(oom_inst_oc4_stringio, "isatty", None),
    )
    del oom_inst_oc4_stringio
    collect()

# OOM sweep: StringIO() constructor
oom_call("oc5:email.iterators.StringIO", getattr(fuzz_target_module, "StringIO", None),
)

oom_inst_oc5_stringio = None
try:
    oom_inst_oc5_stringio = callFunc("oc5_init", "StringIO",
    )
except Exception:
    oom_inst_oc5_stringio = None

if oom_inst_oc5_stringio is not None and oom_inst_oc5_stringio is not SENTINEL_VALUE:
    # OOM sweep: StringIO.__reduce__()
    oom_call("oc5m1:email.iterators.StringIO.__reduce__", getattr(oom_inst_oc5_stringio, "__reduce__", None),
    )
    # OOM sweep: StringIO.readlines()
    oom_call("oc5m2:email.iterators.StringIO.readlines", getattr(oom_inst_oc5_stringio, "readlines", None),
        7,
    )
    # OOM sweep: StringIO.readable()
    oom_call("oc5m3:email.iterators.StringIO.readable", getattr(oom_inst_oc5_stringio, "readable", None),
    )
    # OOM sweep: StringIO.readable()
    oom_call("oc5m4:email.iterators.StringIO.readable", getattr(oom_inst_oc5_stringio, "readable", None),
    )
    # OOM sweep: StringIO.__next__()
    oom_call("oc5m5:email.iterators.StringIO.__next__", getattr(oom_inst_oc5_stringio, "__next__", None),
        errback,
    )
    del oom_inst_oc5_stringio
    collect()
