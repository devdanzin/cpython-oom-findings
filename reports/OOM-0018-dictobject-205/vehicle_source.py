...
print("Importing target module: wsgiref.util", file=stderr)
import wsgiref.util
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"wsgiref.util.{method_name}()" if obj_to_call is wsgiref.util else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, wsgiref.util, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = wsgiref.util
...
print("--- Fuzzing 7 functions in wsgiref.util ---", file=stderr)
# OOM sweep: _hoppish
oom_call(getattr(fuzz_target_module, "_hoppish"),
    '/etc/machine-id',
)
...
# OOM sweep: is_hop_by_hop
oom_call(getattr(fuzz_target_module, "is_hop_by_hop"),
    -7,
)
...
# OOM sweep: shift_path_info
oom_call(getattr(fuzz_target_module, "shift_path_info"),
    MagicMock(),
)

# OOM sweep: setup_testing_defaults
oom_call(getattr(fuzz_target_module, "setup_testing_defaults"),
    None,
)
