...
print("Importing target module: importlib.metadata", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"importlib.metadata.{method_name}()" if obj_to_call is importlib.metadata else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, importlib.metadata, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = importlib.metadata
...
print("--- Fuzzing 20 functions in importlib.metadata ---", file=stderr)
# OOM sweep: _get_toplevel_name
oom_call("f1:importlib.metadata._get_toplevel_name", getattr(fuzz_target_module, "_get_toplevel_name"),
    tuple[weird_classes['weird_Decimal']],
)

# OOM sweep: pass_none
oom_call("f2:importlib.metadata.pass_none", getattr(fuzz_target_module, "pass_none"),
    b"\x48\x7C\x4A\x45\x68\xB6\x81\x87\x19\x48\xAC\x51\x31\x5A\x66\x98\x61",
)

# OOM sweep: entry_points
oom_call("f3:importlib.metadata.entry_points", getattr(fuzz_target_module, "entry_points"),
)

# OOM sweep: _clear_after_fork
oom_call("f4:importlib.metadata._clear_after_fork", getattr(fuzz_target_module, "_clear_after_fork"),
    "\xDA\x89\xB1\xD7\xA9",
)

# OOM sweep: _get_toplevel_name
oom_call("f5:importlib.metadata._get_toplevel_name", getattr(fuzz_target_module, "_get_toplevel_name"),
    weird_classes['weird_Decimal'],
)

# OOM sweep: _top_level_declared
oom_call("f6:importlib.metadata._top_level_declared", getattr(fuzz_target_module, "_top_level_declared"),
    type,
)

# OOM sweep: distribution
oom_call("f7:importlib.metadata.distribution", getattr(fuzz_target_module, "distribution"),
    "\udbff\udfff",
)

# OOM sweep: _topmost
oom_call("f8:importlib.metadata._topmost", getattr(fuzz_target_module, "_topmost"),
    r"isD+srLrYgx.\WcCY\d.clZGtIuYSzvinuoqOa",
)

# OOM sweep: method_cache
oom_call("f9:importlib.metadata.method_cache", getattr(fuzz_target_module, "method_cache"),
    tuple[weird_classes['weird_deque']] | weird_classes['weird_OrderedDict'] | big_union,
)

# OOM sweep: requires
oom_call("f10:importlib.metadata.requires", getattr(fuzz_target_module, "requires"),
    0,
)
