...
print("Importing target module: _blake2", file=stderr)
import _blake2
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"_blake2.{method_name}()" if obj_to_call is _blake2 else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, _blake2, func_name_str, *arguments, verbose=verbose)
...
print("--- OOM-fuzzing 2 classes in _blake2 ---", file=stderr)
# OOM sweep: blake2b() constructor
oom_call("oc1:_blake2.blake2b", getattr(fuzz_target_module, "blake2b", None),
)

oom_inst_oc1_blake2b = None
try:
    oom_inst_oc1_blake2b = callFunc("oc1_init", "blake2b",
    )
except Exception:
    oom_inst_oc1_blake2b = None

if oom_inst_oc1_blake2b is not None and oom_inst_oc1_blake2b is not SENTINEL_VALUE:
    # OOM sequence on blake2b: update > __dir__
    def _oom_seq_oc1():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:update", file=stderr)
            getattr(oom_inst_oc1_blake2b, "update", None)(
                "s\xB7\x88",
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc1_blake2b, "__dir__", None)(
            )
        except BaseException:
            pass
    oom_run("oc1:_blake2.blake2b[update>__dir__]", _oom_seq_oc1, window=1)

    del oom_inst_oc1_blake2b
    collect()

# OOM sweep: blake2s() constructor
oom_call("oc2:_blake2.blake2s", getattr(fuzz_target_module, "blake2s", None),
)

oom_inst_oc2_blake2s = None
try:
    oom_inst_oc2_blake2s = callFunc("oc2_init", "blake2s",
    )
except Exception:
    oom_inst_oc2_blake2s = None

if oom_inst_oc2_blake2s is not None and oom_inst_oc2_blake2s is not SENTINEL_VALUE:
    # OOM sequence on blake2s: copy > __reduce__ > __init__ > __getstate__ > __init_subclass__
    def _oom_seq_oc2():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:copy", file=stderr)
            getattr(oom_inst_oc2_blake2s, "copy", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:__reduce__", file=stderr)
            getattr(oom_inst_oc2_blake2s, "__reduce__", None)(
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc2_blake2s, "__init__", None)(
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc2_blake2s, "__getstate__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m5:__init_subclass__", file=stderr)
            getattr(oom_inst_oc2_blake2s, "__init_subclass__", None)(
                9,
            )
        except BaseException:
            pass
    oom_run("oc2:_blake2.blake2s[copy>__reduce__>__init__>__getstate__>__init_subclass__]", _oom_seq_oc2, window=3)

    del oom_inst_oc2_blake2s
    collect()

# OOM sweep: blake2s() constructor
oom_call("oc3:_blake2.blake2s", getattr(fuzz_target_module, "blake2s", None),
)

oom_inst_oc3_blake2s = None
try:
    oom_inst_oc3_blake2s = callFunc("oc3_init", "blake2s",
    )
except Exception:
    oom_inst_oc3_blake2s = None

if oom_inst_oc3_blake2s is not None and oom_inst_oc3_blake2s is not SENTINEL_VALUE:
    # OOM sequence on blake2s: __eq__ > __ge__ > copy > update > __repr__ > update
    def _oom_seq_oc3():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__eq__", file=stderr)
            getattr(oom_inst_oc3_blake2s, "__eq__", None)(
                False,
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc3_blake2s, "__ge__", None)(
                list[weird_classes['weird_object']] | weird_classes['weird_int'] | big_union,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m3:copy", file=stderr)
            getattr(oom_inst_oc3_blake2s, "copy", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m4:update", file=stderr)
            getattr(oom_inst_oc3_blake2s, "update", None)(
                -266.900,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m5:__repr__", file=stderr)
            getattr(oom_inst_oc3_blake2s, "__repr__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m6:update", file=stderr)
            getattr(oom_inst_oc3_blake2s, "update", None)(
                [[[[[[[[[[[[[[]]]]]]]]]]]]]],
            )
        except BaseException:
            pass
    oom_run("oc3:_blake2.blake2s[__eq__>__ge__>copy>update>__repr__>update]", _oom_seq_oc3, window=4)

    del oom_inst_oc3_blake2s
    collect()

# OOM sweep: blake2b() constructor
oom_call("oc4:_blake2.blake2b", getattr(fuzz_target_module, "blake2b", None),
)

oom_inst_oc4_blake2b = None
try:
    oom_inst_oc4_blake2b = callFunc("oc4_init", "blake2b",
    )
except Exception:
    oom_inst_oc4_blake2b = None

if oom_inst_oc4_blake2b is not None and oom_inst_oc4_blake2b is not SENTINEL_VALUE:
    # OOM sequence on blake2b: copy > __reduce_ex__ > __subclasshook__ > __gt__ > __init__ > __init_subclass__
    def _oom_seq_oc4():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:copy", file=stderr)
            getattr(oom_inst_oc4_blake2b, "copy", None)(
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc4_blake2b, "__reduce_ex__", None)(
                DescriptorBomb(),
            )
        except BaseException:
            pass
        try:
            ...
            getattr(oom_inst_oc4_blake2b, "__subclasshook__", None)(
                2313370004098379,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m4:__gt__", file=stderr)
            getattr(oom_inst_oc4_blake2b, "__gt__", None)(
                HiddenNameType,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m5:__init__", file=stderr)
            getattr(oom_inst_oc4_blake2b, "__init__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m6:__init_subclass__", file=stderr)
            getattr(oom_inst_oc4_blake2b, "__init_subclass__", None)(
                FailingIterator(),
            )
        except BaseException:
            pass
    oom_run("oc4:_blake2.blake2b[copy>__reduce_ex__>__subclasshook__>__gt__>__init__>__init_subclass__]", _oom_seq_oc4, window=2)

    del oom_inst_oc4_blake2b
    collect()

# OOM sweep: blake2s() constructor
oom_call("oc5:_blake2.blake2s", getattr(fuzz_target_module, "blake2s", None),
)

oom_inst_oc5_blake2s = None
try:
    oom_inst_oc5_blake2s = callFunc("oc5_init", "blake2s",
    )
except Exception:
    oom_inst_oc5_blake2s = None

if oom_inst_oc5_blake2s is not None and oom_inst_oc5_blake2s is not SENTINEL_VALUE:
    # OOM sequence on blake2s: __eq__ > __new__ > __ge__ > __hash__ > __dir__
    def _oom_seq_oc5():
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m1:__eq__", file=stderr)
            getattr(oom_inst_oc5_blake2s, "__eq__", None)(
                FilenoBomb(),
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m2:__new__", file=stderr)
            getattr(oom_inst_oc5_blake2s, "__new__", None)(
                ReprBomb(),
                SuperBomb(),
                EqBomb(),
            )
        ...
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m3:__ge__", file=stderr)
            getattr(oom_inst_oc5_blake2s, "__ge__", None)(
                tricky_capsule,
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m4:__hash__", file=stderr)
            getattr(oom_inst_oc5_blake2s, "__hash__", None)(
            )
        except BaseException:
            pass
        try:
            if _OOM_VERBOSE:
                print("[OOM-SEQ]     step m5:__dir__", file=stderr)
            getattr(oom_inst_oc5_blake2s, "__dir__", None)(
            )
        except BaseException:
            pass
    oom_run("oc5:_blake2.blake2s[__eq__>__new__>__ge__>__hash__>__dir__]", _oom_seq_oc5, window=3)

    del oom_inst_oc5_blake2s
    collect()
