...
print("Importing target module: _pyrepl.utils", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"_pyrepl.utils.{method_name}()" if obj_to_call is _pyrepl.utils else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, _pyrepl.utils, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = _pyrepl.utils
...
print("--- Fuzzing 14 functions in _pyrepl.utils ---", file=stderr)
# OOM sweep: gen_colors_from_token_stream
oom_call("f1:_pyrepl.utils.gen_colors_from_token_stream", getattr(fuzz_target_module, "gen_colors_from_token_stream"),
    errback,
    weird_instances['weird_int_sys_maxsize_plus_one'],
)

# OOM sweep: prev_next_window
oom_call("f2:_pyrepl.utils.prev_next_window", getattr(fuzz_target_module, "prev_next_window"),
    errback,
)

# OOM sweep: THEME
oom_call("f3:_pyrepl.utils.THEME", getattr(fuzz_target_module, "THEME"),
)

# OOM sweep: iter_display_chars
oom_call("f4:_pyrepl.utils.iter_display_chars", getattr(fuzz_target_module, "iter_display_chars"),
    False,
)

# OOM sweep: _ascii_control_repr
oom_call("f5:_pyrepl.utils._ascii_control_repr", getattr(fuzz_target_module, "_ascii_control_repr"),
    tuple[weird_classes['weird_frozenset']],
)

# OOM sweep: _ascii_control_repr
oom_call("f6:_pyrepl.utils._ascii_control_repr", getattr(fuzz_target_module, "_ascii_control_repr"),
    "pk&K\xB65\xF6",
)

# OOM sweep: iter_display_chars
oom_call("f7:_pyrepl.utils.iter_display_chars", getattr(fuzz_target_module, "iter_display_chars"),
    "\x00",
    errback,
    "u",
)
...
# OOM sweep: trace
oom_call("f8:_pyrepl.utils.trace", getattr(fuzz_target_module, "trace"),
    "/y/",
)

# OOM sweep: disp_str
oom_call("f9:_pyrepl.utils.disp_str", getattr(fuzz_target_module, "disp_str"),
    "\x00\x0CN\x04\x8A\xD5\x03",
)

# OOM sweep: iter_display_chars
oom_call("f10:_pyrepl.utils.iter_display_chars", getattr(fuzz_target_module, "iter_display_chars"),
    Evil(),
    18.69,
)
