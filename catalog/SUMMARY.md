...
| OOM-0006 | `dictiter_dealloc` dict-iterator dealloc under OOM | abort | release | no | `dictobject.c:dictiter_dealloc` |
| OOM-0007 | `context_tp_dealloc` with a pending exception | fatal | ASan/jit | no | `context.c:context_tp_dealloc` |
...
| OOM-0011 | `LOAD_ATTR` specialization under OOM | abort | ASan/jit | no | `specialize.c:specialize` |
...
| OOM-0017 | free-threaded GC `gc_get_refs` assert (alignment-sensitive) | abort | ASan/jit | no | `gc_free_threading.c:validate_gc_objects` |
| OOM-0018 | dict `set_keys`: managed-dict freed cross-thread under OOM (racy) | abort | debug | no | `dictobject.c:set_keys` |
...
| OOM-0022 | slot succeeded with an exception set (extension reload) | fatal | ASan/jit | no | `call.c:_Py_CheckSlotResult` |
