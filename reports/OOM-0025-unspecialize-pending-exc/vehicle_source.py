...
print("Importing target module: sys", file=stderr)
import sys
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"sys.{method_name}()" if obj_to_call is sys else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, sys, func_name_str, *arguments, verbose=verbose)
...
print("--- Fuzzing 63 functions in sys ---", file=stderr)
# OOM sweep: _baserepl
oom_call("f1:sys._baserepl", getattr(fuzz_target_module, "_baserepl"),
)

# OOM sweep: _baserepl
oom_call("f2:sys._baserepl", getattr(fuzz_target_module, "_baserepl"),
)

# OOM sweep: breakpointhook
oom_call("f3:sys.breakpointhook", getattr(fuzz_target_module, "breakpointhook"),
)

# OOM sweep: _current_exceptions
oom_call("f4:sys._current_exceptions", getattr(fuzz_target_module, "_current_exceptions"),
)

# OOM sweep: remote_exec
oom_call("f5:sys.remote_exec", getattr(fuzz_target_module, "remote_exec"),
    17,
    complex(-10 ** 10, 10 ** 10),
)

# OOM sweep: getsizeof
oom_call("f6:sys.getsizeof", getattr(fuzz_target_module, "getsizeof"),
    b"\x00" * (2**16),
)

# OOM sweep: getunicodeinternedsize
oom_call("f7:sys.getunicodeinternedsize", getattr(fuzz_target_module, "getunicodeinternedsize"),
)

# OOM sweep: _is_immortal
oom_call("f8:sys._is_immortal", getattr(fuzz_target_module, "_is_immortal"),
    r"JEVD\BCjnHlWsLFRcJYk*\DMjGMs..\A\wrMk\Z\Wvd.N.A..",
)

# OOM sweep: _is_interned
oom_call("f9:sys._is_interned", getattr(fuzz_target_module, "_is_interned"),
    -57879500,
)

# OOM sweep: setrecursionlimit
oom_call("f10:sys.setrecursionlimit", getattr(fuzz_target_module, "setrecursionlimit"),
