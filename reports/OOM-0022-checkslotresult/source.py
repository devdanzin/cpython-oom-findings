...
# OOM sweep: lasti2lineno
oom_call("f1:pdb.lasti2lineno", getattr(fuzz_target_module, "lasti2lineno"),
    {"\xA0\x94o\xB4,": weird_instances['weird_complex_2**31-1'],
     b"\x1D\xA8\xB6\x39\x18\x7B\x47\x39\x80\x76\x6A\x15\x12\xA3\x48\xA0\xFC": "..//ob/./../NqaAHW/z4",
     "\x17u^": 52.187,
     "\U0010FFFF": "\uB10F\u9F32\u031E\u4414\u45FE\u12CB\uEEB5\u462F\u43EF\u52C9\u757C",
     None: errback,
     6.10: Exception('fuzzer_generated_exception'),
     118.9944: "\x00"},
    Template("\x00", Interpolation(2 ** 63 - 1, "value")),
)

# OOM sweep: set_trace
oom_call("f2:pdb.set_trace", getattr(fuzz_target_module, "set_trace"),
)

# OOM sweep: lasti2lineno
oom_call("f3:pdb.lasti2lineno", getattr(fuzz_target_module, "lasti2lineno"),
    "\x13\x15\xB4",
    memoryview(bytearray(b"abc\xe9\xff")),
)

# OOM sweep: pm
oom_call("f4:pdb.pm", getattr(fuzz_target_module, "pm"),
)

# OOM sweep: _post_mortem
oom_call("f5:pdb._post_mortem", getattr(fuzz_target_module, "_post_mortem"),
    tricky_function,
    dict[weird_classes['weird_str']],
)

# OOM sweep: attach
oom_call("f6:pdb.attach", getattr(fuzz_target_module, "attach"),
    MagicMock(),
    r"rLV?QDX.f",
)

# OOM sweep: _connect
oom_call("f7:pdb._connect", getattr(fuzz_target_module, "_connect"),
)

# OOM sweep: _pyrepl_available
oom_call("f8:pdb._pyrepl_available", getattr(fuzz_target_module, "_pyrepl_available"),
)

# OOM sweep: runctx
oom_call("f9:pdb.runctx", getattr(fuzz_target_module, "runctx"),
    Exception('fuzzer_generated_exception'),
    Exception('fuzzer_generated_exception'),
    None,
)

# OOM sweep: exit_with_permission_help_text
oom_call("f10:pdb.exit_with_permission_help_text", getattr(fuzz_target_module, "exit_with_permission_help_text"),
)
