...
print("Importing target module: xml.dom.minidom", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"xml.dom.minidom.{method_name}()" if obj_to_call is xml.dom.minidom else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, xml.dom.minidom, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = xml.dom.minidom
...
print("--- Fuzzing 15 functions in xml.dom.minidom ---", file=stderr)
# OOM sweep: _write_data
oom_call(getattr(fuzz_target_module, "_write_data"),
    MagicMock,
    r"LKkX.AihE.U\W.",
    list[weird_classes['weird_str']] | weird_classes['weird_Decimal'] | big_union,
)

# OOM sweep: _clone_node
oom_call(getattr(fuzz_target_module, "_clone_node"),
    b"\x73\xBB\x61\x4C\xDE\x87\x9B\x2E\x22\x90\x5D\xF3\x82\xFF\x5A\xD0\xAB\x5D\x2F",
    109932173,
    weird_instances['weird_Decimal_2**31'],
)

# OOM sweep: _nssplit
oom_call(getattr(fuzz_target_module, "_nssplit"),
    list[weird_classes['weird_deque']],
)

# OOM sweep: _do_pulldom_parse
oom_call(getattr(fuzz_target_module, "_do_pulldom_parse"),
    "\udbff\udfff",
    "\x11\xA5\x00\x18\xE4\xA2",
    '/etc/machine-id',
)

# OOM sweep: _get_containing_entref
oom_call(getattr(fuzz_target_module, "_get_containing_entref"),
    tricky_function,
)

# OOM sweep: _clear_id_cache
oom_call(getattr(fuzz_target_module, "_clear_id_cache"),
    r"\d{1,100}",
)
...
# OOM sweep: parse
oom_call(getattr(fuzz_target_module, "parse"),
    b"\x40\xAC\x13\xA4\x22",
    dict[weird_classes['weird_deque']],
    b"\xC3\x27\x45\x9F\x6B",
)
...
# OOM sweep: _get_containing_element
oom_call(getattr(fuzz_target_module, "_get_containing_element"),
    inspect,
)
