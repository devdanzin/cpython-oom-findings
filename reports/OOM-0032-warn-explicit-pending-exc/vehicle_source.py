...
print("Importing target module: zoneinfo._tzpath", file=stderr)
import zoneinfo._tzpath
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"zoneinfo._tzpath.{method_name}()" if obj_to_call is zoneinfo._tzpath else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, zoneinfo._tzpath, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = zoneinfo._tzpath
...
print("--- Fuzzing 7 functions in zoneinfo._tzpath ---", file=stderr)
# OOM sweep: _get_invalid_paths_message
oom_call("f1:zoneinfo._tzpath._get_invalid_paths_message", getattr(fuzz_target_module, "_get_invalid_paths_message"),
    True,
)

# OOM sweep: _parse_python_tzpath
oom_call("f2:zoneinfo._tzpath._parse_python_tzpath", getattr(fuzz_target_module, "_parse_python_tzpath"),
    "\x1C\x86\xF9\xA2G",
    weird_classes['weird_Decimal'],
)

# OOM sweep: find_tzfile
oom_call("f3:zoneinfo._tzpath.find_tzfile", getattr(fuzz_target_module, "find_tzfile"),
    Exception('fuzzer_generated_exception'),
)

# OOM sweep: _parse_python_tzpath
oom_call("f4:zoneinfo._tzpath._parse_python_tzpath", getattr(fuzz_target_module, "_parse_python_tzpath"),
    '/bin/sh',
    errback,
)

# OOM sweep: _parse_python_tzpath
oom_call("f5:zoneinfo._tzpath._parse_python_tzpath", getattr(fuzz_target_module, "_parse_python_tzpath"),
    weird_classes['weird_bytearray'],
    -15,
)

# OOM sweep: find_tzfile
oom_call("f6:zoneinfo._tzpath.find_tzfile", getattr(fuzz_target_module, "find_tzfile"),
    dict[weird_classes['weird_dict']] | weird_classes['weird_Counter'] | big_union,
)

# OOM sweep: find_tzfile
oom_call("f7:zoneinfo._tzpath.find_tzfile", getattr(fuzz_target_module, "find_tzfile"),
    -11,
)

# OOM sweep: _parse_python_tzpath
oom_call("f8:zoneinfo._tzpath._parse_python_tzpath", getattr(fuzz_target_module, "_parse_python_tzpath"),
    tuple[weird_classes['weird_set']] | weird_classes['weird_OrderedDict'] | big_union,
    tricky_function,
)

# OOM sweep: _get_invalid_paths_message
oom_call("f9:zoneinfo._tzpath._get_invalid_paths_message", getattr(fuzz_target_module, "_get_invalid_paths_message"),
    r"\BrC.N.ep+\DQ?\SRtoOqijvxSzO\bolhQ\DIIiZSBrK\w\BG\D",
)

# OOM sweep: available_timezones
oom_call("f10:zoneinfo._tzpath.available_timezones", getattr(fuzz_target_module, "available_timezones"),
)
