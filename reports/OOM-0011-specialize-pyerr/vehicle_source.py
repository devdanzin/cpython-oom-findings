...
print("Importing target module: optparse", file=stderr)
import optparse
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"optparse.{method_name}()" if obj_to_call is optparse else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, optparse, func_name_str, *arguments, verbose=verbose)
...
print("--- Fuzzing 9 functions in optparse ---", file=stderr)
# OOM sweep: _repr
oom_call("f1:optparse._repr", getattr(fuzz_target_module, "_repr"),
)

# OOM sweep: _
oom_call("f2:optparse._", getattr(fuzz_target_module, "_"),
    weird_instances['weird_OrderedDict_tricky_strs'],
)

# OOM sweep: ngettext
oom_call("f3:optparse.ngettext", getattr(fuzz_target_module, "ngettext"),
    8.605,
    r"mY\A.XyvhBweLPmL.I\B.EI",
    "p\xD4\xB7\xF1X\x01w",
)

# OOM sweep: ngettext
oom_call("f4:optparse.ngettext", getattr(fuzz_target_module, "ngettext"),
    [[[[[[[[[[[[[[]]]]]]]]]]]]]],
    r"LuF.GE\BScBBxohQ*Kp*Lr\dDj.z\sci.OR?Xuq.soP",
    None,
)

# OOM sweep: _
oom_call("f5:optparse._", getattr(fuzz_target_module, "_"),
    "\U0010FFFF",
)

# OOM sweep: check_choice
oom_call("f6:optparse.check_choice", getattr(fuzz_target_module, "check_choice"),
    8,
    errback,
    MagicMock(),
)

# OOM sweep: check_builtin
oom_call("f7:optparse.check_builtin", getattr(fuzz_target_module, "check_builtin"),
    "\x94h\xB3\x19\xE3\x81\x82\x9A\xC8\x9B\xF0+\xCB\xD69\x87\xC3\x06\xF8\xB4",
    Exception('fuzzer_generated_exception'),
    errback,
)

# OOM sweep: _parse_int
oom_call("f8:optparse._parse_int", getattr(fuzz_target_module, "_parse_int"),
    weird_instances['weird_deque_printable'],
)

# OOM sweep: _repr
oom_call("f9:optparse._repr", getattr(fuzz_target_module, "_repr"),
)

# OOM sweep: _repr
oom_call("f10:optparse._repr", getattr(fuzz_target_module, "_repr"),
)
