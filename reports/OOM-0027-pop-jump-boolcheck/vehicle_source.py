...
print("Importing target module: _pyrepl.windows_eventqueue", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"_pyrepl.windows_eventqueue.{method_name}()" if obj_to_call is _pyrepl.windows_eventqueue else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, _pyrepl.windows_eventqueue, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = _pyrepl.windows_eventqueue
...
print("--- Fuzzing 1 functions in _pyrepl.windows_eventqueue ---", file=stderr)
# OOM sweep: __annotate__
oom_call("f1:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    sys.float_info.min,
)

# OOM sweep: __annotate__
oom_call("f2:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    weird_classes['weird_dict'],
)

# OOM sweep: __annotate__
oom_call("f3:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    liar2,
)

# OOM sweep: __annotate__
oom_call("f4:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    "\uF99A\uC8AA\u7E87\u5948",
)

# OOM sweep: __annotate__
oom_call("f5:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    "\uD74C\u8FE5\uB54F\uD273\u911A\u7C37",
)

# OOM sweep: __annotate__
oom_call("f6:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    list[weird_classes['weird_int']],
)

# OOM sweep: __annotate__
oom_call("f7:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    Exception('fuzzer_generated_exception'),
)

# OOM sweep: __annotate__
oom_call("f8:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    Exception('fuzzer_generated_exception'),
)

# OOM sweep: __annotate__
oom_call("f9:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    MagicMock(),
)

# OOM sweep: __annotate__
oom_call("f10:_pyrepl.windows_eventqueue.__annotate__", getattr(fuzz_target_module, "__annotate__"),
    -14,
)
