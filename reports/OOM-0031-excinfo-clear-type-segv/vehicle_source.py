...
print("Importing target module: _interpreters", file=stderr)
...
def callMethod(prefix, obj_to_call, method_name, *arguments, verbose=True):
    func_display_name = f"_interpreters.{method_name}()" if obj_to_call is _interpreters else f"{obj_to_call.__class__.__name__}.{method_name}()"
...
def callFunc(prefix, func_name_str, *arguments, verbose=True):
    return callMethod(prefix, _interpreters, func_name_str, *arguments, verbose=verbose)
...
fuzz_target_module = _interpreters
...
print("--- Fuzzing 18 functions in _interpreters ---", file=stderr)
# OOM sweep: exec
oom_call("f1:_interpreters.exec", getattr(fuzz_target_module, "exec"),
    (sys.maxunicode + 1,),
    errback,
    tuple[weird_classes['weird_dict']],
)

# OOM sweep: exec
oom_call("f2:_interpreters.exec", getattr(fuzz_target_module, "exec"),
    95.5,
    bytearray(b"test"),
    "\xBC\x98\xB6\xB9\xF2",
)

# OOM sweep: get_current
oom_call("f3:_interpreters.get_current", getattr(fuzz_target_module, "get_current"),
)

# OOM sweep: capture_exception
oom_call("f4:_interpreters.capture_exception", getattr(fuzz_target_module, "capture_exception"),
    "7g0NO9msMi1opQdOtxPIw_GNQ.uL2WdjguoHmQBUwV_qSf-UYaryQtwhMyrFtArzSeVj2zFv/..",
)

# OOM sweep: list_all
oom_call("f5:_interpreters.list_all", getattr(fuzz_target_module, "list_all"),
)

# OOM sweep: capture_exception
oom_call("f6:_interpreters.capture_exception", getattr(fuzz_target_module, "capture_exception"),
    Exception('fuzzer_generated_exception'),
)

# OOM sweep: incref
oom_call("f7:_interpreters.incref", getattr(fuzz_target_module, "incref"),
    tricky_traceback,
)

# OOM sweep: create
oom_call("f8:_interpreters.create", getattr(fuzz_target_module, "create"),
    {r"QoGaU.HG\AbHxgOh.\BdcX\DH"},
)

# OOM sweep: decref
oom_call("f9:_interpreters.decref", getattr(fuzz_target_module, "decref"),
    b"\xD2\xEF\xC9\x2A\x99",
)

# OOM sweep: is_shareable
oom_call("f10:_interpreters.is_shareable", getattr(fuzz_target_module, "is_shareable"),
    "AY\xD1\xF2\xB7",
)
